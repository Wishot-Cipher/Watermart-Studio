export async function drawWatermarkOnCanvas(canvasRef, faceModel, baseUrl, opts) {
    return new Promise((resolve, reject) => {
        ...
        img.onload = async () => {
            ...
            const filters = [];
            if (opts.exposure !== 0)
                filters.push(`brightness(${1 + opts.exposure / 100})`);
            if (opts.contrast !== 0)
                filters.push(`contrast(${1 + opts.contrast / 100})`);
            if (opts.saturation !== 0)
                filters.push(`saturate(${1 + opts.saturation / 100})`);
            if (opts.temperature !== 0)
                filters.push(`hue-rotate(${opts.temperature * 0.3}deg)`);
            ...
            if (opts.color) {
                try {
                    watermarkColorRgb = hexToRgb(opts.color);
                }
                catch {
                    watermarkColorRgb = '255,255,255';
                }
            }
            ...
            try {
                ...
                if (autoPlaceRequested && faceModel) {
                    ...
                    if (predictions && predictions.length > 0) {
                        ...
                        const makeBoxFor = (c) => c === 'top-left' ? { x: 0, y: 0, width: logicalW / 3, height: logicalH / 3 }
            ...
            if (opts.customPos && typeof opts.customPos.x === 'number' && typeof opts.customPos.y === 'number') {
                ...
                switch (preferPosition) {
                    case 'top-left':
                        ...
                        break;
                    case 'top-right':
                        x = logicalW - 20 - textWidth;
                        y = fontSize + 20;
                        break;
                    case 'center':
                        x = (logicalW - textWidth) / 2;
                        y = (logicalH / 2) + (fontSize / 2);
                        break;
                    case 'bottom-left':
                        x = 20;
                        y = logicalH - 20;
