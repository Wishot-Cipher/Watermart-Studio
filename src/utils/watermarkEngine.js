...
// ===================================================================
import { EXPORT_QUALITY_SETTINGS } from '@/types/watermark';
const STYLE_CONFIGS = {
    'modern-glass': {
 ...
 */
export async function renderWatermark(imageUrl, config, adjustments, faceModel, customPosition, 
...
exportOptions) {
    return new Promise((resolve, reject) => {
        ...
        img.onload = async () => {
            try {
                console.log('renderWatermark: adjustments.filterPreset=', adjustments?.filterPreset, 'exportOptions=', exportOptions);
            }
            catch (err) {
                console.debug(err);
            }
            try {
                ...
                if (!exportOptions) {
                    ...
                    qualityKey = (exportOptions in EXPORT_QUALITY_SETTINGS) ? exportOptions : 'standard';
                }
                else {
                ...
                if (!settings) {
                    if (exportOptions === 'small') {
                        settings = { quality: 0.7, maxDimension: 1280 };
                    }
                    else {
                ...
                let GLOBAL_MAX_EXPORT_DIMENSION;
                if (qualityKey === 'ultra') {
                ...
                const logosToDraw = Array.isArray(config.logos)
                    ? config.logos.map((l, idx) => {
...
function applyFinalSharpen(ctx, canvas, amount) {
    try {
 ...
 */
function applyImageAdjustments(ctx, img, adj, width, height) {
 ...
 */
function applyTonalAdjustments(ctx, width, height, adj) {
 ...
 */
async function calculatePosition(canvas, config, faceModel, customPosition) {
    if (customPosition) {
        return {
    ...
    if (config.aiPlacement && faceModel) {
        try {
            ...
            if (faces.length > 0) {
                const face = faces[0];
                const resolvePoint = async (pt) => {
                    if (Array.isArray(pt) && pt.length >= 2)
                        return [pt[0], pt[1]];
                    if (pt?.array)
                        return await pt.array();
                    if (pt?.dataSync) {
 ...
 */
async function drawTextWatermark(ctx, canvas, config, position) {
    ...
    if (styleConfig.textTransform === 'lowercase')
 ...
 */
async function drawMultipleLogos(ctx, canvas, logos, canvasWidth, canvasHeight, faceModel) {
    ...
    const isSafe = (x, y) => {
        ...
        for (const b of faceBoxes) {
            if (x >= b.x1 - margin && x <= b.x2 + margin && y >= b.y1 - margin && y <= b.y2 + margin)
                return false;
        }
        ...
        for (const p of chosenPositions) {
            const dx = p.x - x;
            const dy = p.y - y;
            if (Math.sqrt(dx * dx + dy * dy) < Math.min(baseSize, 160))
                return false;
        }
        // safe
        return true;
    ...
    for (const logo of logos) {
        ...
        if (typeof targetX === 'number' && typeof targetY === 'number') {
            ...
            const adjustedLogo = { ...logo, position: { x: targetX, y: targetY } };
 ...
 */
function drawSingleLogo(ctx, logo, canvasWidth, canvasHeight, baseSizeOverride) {
    return new Promise((resolve, reject) => {
        const img = new Image();
        img.crossOrigin = 'anonymous';
        img.onload = () => {
            try {
                ...
                let normY = rawY;
                if (rawX <= 1)
                    normX = Math.min(1 - marginPercent, Math.max(marginPercent, rawX));
                if (rawY <= 1)
                    normY = Math.min(1 - marginPercent, Math.max(marginPercent, rawY));
        ...
        img.src = logo.dataUrl || logo.url || '';
    });
}
function drawBackground(ctx, styleConfig, x, y, width, height) {
...
function drawBorder(ctx, styleConfig, x, y, width, height) {
