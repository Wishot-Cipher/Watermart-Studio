...
export default function WatermarkPreview(props) {
    const { watermarkType, watermarkText, logos, setLogos, previewImgWidth, size, watermarkColor, fontWeightState, fontFamily, glowEffect, position, customPos, setCustomPos, setIsDragging, isDragging, setPosition, containerRef, blendMode, opacity, style, rotation, strokeWidth, strokeColor, flushRef } = props;
    useEffect(() => {
    ...
    const getBoxShadow = () => {
        if (glowEffect)
            return '0 0 25px rgba(26, 124, 255, 0.8), 0 0 45px rgba(26, 124, 255, 0.4)';
        if (style === 'modern-glass')
            return '0 8px 32px rgba(0, 0, 0, 0.15)';
        if (style === 'neon-glow')
            return '0 0 25px rgba(0, 255, 255, 0.6)';
        if (style === 'minimal-clean')
            return '0 4px 16px rgba(0, 0, 0, 0.1)';
        if (style === 'gradient-fade')
            return '0 6px 24px rgba(26, 124, 255, 0.4)';
        if (style === 'tech-futuristic')
            return '0 0 20px rgba(0, 255, 157, 0.4)';
        return undefined;
    };
    ...
    React.useEffect(() => {
        ...
        flushRef.current = () => {
            ...
            try {
                ...
                console.debug('flush pointer failed', err);
            }
            ...
            try {
                ...
                const els = Array.from(container.querySelectorAll('.wm-logo-draggable'));
                if (!els || els.length === 0)
                    return;
    ...
    return (_jsx(AnimatePresence, { children: watermarkType !== 'none' && (_jsxs(motion.div, { initial: { opacity: 0 }, animate: { opacity: opacity / 100 }, exit: { opacity: 0 }, transition: { duration: 0.2 }, className: `absolute ${position === 'custom' ? '' : currentPosition} z-30 pointer-events-auto cursor-grab active:cursor-grabbing select-none`, onPointerDown: (e) => {
                setIsDragging(true);
                try {
                ...
                pointerEvents: 'auto',
                touchAction: 'none'
            }, children: [_jsx("div", { className: `${styleConfig.bg} ${styleConfig.border} ${styleConfig.radius} ${styleConfig.padding} transition-all duration-200`, style: {
                        boxShadow: getBoxShadow()
                    }, children: _jsx("div", { className: `flex items-center gap-3 ${textEffects}`, style: {
                            ...
                            filter: textShadow,
                            lineHeight: 1.2
                        }, children: (watermarkType === 'text' || watermarkType === 'both') && (_jsx("span", { style: {
                                color: previewColor,
                                fontWeight: Number(fontWeightState),
                            ...
                            }, children: watermarkText })) }) }), (watermarkType === 'logo' || watermarkType === 'both') && logos && logos.length > 0 && logos.map((l, i) => {
                    ...
                    return (_jsxs("div", { "data-index": i, className: "wm-logo-draggable absolute z-50 cursor-grab active:cursor-grabbing", style: {
                            width: sizePx,
                            height: sizePx,
                            left,
                            top,
                            ...
                            try {
                                ...
                                console.debug('setPointerCapture', err);
                            }
                            ...
                            try {
                                logoEl.style.touchAction = 'none';
                            }
                            catch (err) {
                                console.debug('set touchAction failed', err);
                            }
                            const move = (ev) => {
                                ...
                                try {
                                    ...
                                    console.debug('logo dom update failed', err);
                                }
                                if (rafId == null) {
                            ...
                            const up = () => {
                                try {
                                    ...
                                    console.debug('releasePointerCapture', err);
                                }
                                if (rafId) {
                                    cancelAnimationFrame(rafId);
                                    rafId = null;
                        ...
                        }, children: [_jsx("img", { src: l.dataUrl, alt: `logo-${i}`, className: "object-contain w-full h-full", style: { display: 'block' } }), _jsxs("div", { className: "absolute -top-2 -right-2 bg-black/70 text-white text-[10px] px-1 rounded-md border border-white/20", children: [sizePx, "px"] })] }, l.id || i));
                })] })) }));
}
